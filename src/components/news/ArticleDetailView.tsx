    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <button
          ...
          className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-700 hover:text-orange-600 bg-white hover:bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-xl transition shadow-xs cursor-pointer group"
        >
          <svg
      ...
      {/* Main Content Card */}
      <article className="bg-white rounded-3xl border border-slate-200 shadow-xs p-6 sm:p-10 space-y-6">
        {/* Title */}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 pb-2">
          <Link
            href="/"
            className="bg-[#9c6a3a] hover:bg-[#83562a] text-white text-[11px] font-semibold px-3 py-1 rounded transition shadow-xs"
          >
            Home
          </Link>
          <Link
          ...
          <span className="bg-[#0b9758] text-white text-[11px] font-semibold px-3 py-1 rounded shadow-xs max-w-full sm:max-w-md truncate">
            {article.title}
          </span>
        </div>
        ...
        {mainPhoto && (
          <div
            ...
            className="relative w-full aspect-video sm:aspect-16/10 max-h-[520px] rounded-2xl overflow-hidden bg-slate-100 border border-slate-200/80 cursor-pointer group shadow-xs"
          >
            <Image
              src={mainPhoto}
              alt={article.title}
        ...
        <div className="text-slate-700 text-base sm:text-lg leading-relaxed space-y-4 pt-2">
          {paragraphs.length > 0 ? (
            paragraphs.map((p, idx) => (
              <p key={idx} className="whitespace-pre-line text-justify sm:text-left leading-relaxed">
                {p}
              </p>
            ))
          ) : (
              ...
              {additionalPhotos.map((img, idx) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                    <span className="bg-black/70 backdrop-blur-xs text-white text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1">
        ...
        <div className="pt-8 mt-8 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <button
            ...
            className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-700 hover:text-orange-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-4 py-2 rounded-xl transition cursor-pointer"
          >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Bagikan:</span>
            <a
